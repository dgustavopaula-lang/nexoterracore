...
app.post("/api/turing/publico", limiteTuring, async (req, res, next) => {
  try {
    const pergunta =
...
app.get("/api/fazendas", autenticar, autorizar("fazendas", "GET"), async (req, res) => {
  try {
    const resultado = await pool.query(
      `
        SELECT
    ...
    console.error(erro);
    res.status(500).json({ erro: "Não foi possível consultar as fazendas." });
  }
});
